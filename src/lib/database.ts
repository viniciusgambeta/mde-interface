...
export const profileService = {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          id: profile.id,
          name: profile.name,
          avatar_url: profile.avatar_url || '/src/images/avatar.jpg',
  ...
  // Get profiles with pagination (for admin or leaderboard)
  async getProfiles(options: {
    limit?: number;
    offset?: number;
    orderBy?: 'score' | 'created_at' | 'name';
    ascending?: boolean;
  } = {}): Promise<Profile[]> {
  ...
  // Update user score
  async updateScore(userId: string, scoreChange: number): Promise<boolean> {
    if (!userId) return false;
    ...
    try {
      ...
      return await this.updateProfile(userId, { score: newScore });
    } catch (error) {
