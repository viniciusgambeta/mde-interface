...
export const commentsService = {
  ...
  async getVideoComments(videoId: string): Promise<Comment[]> {
    ...
    try {
      ...
      // Transform and load replies for each comment
      const comments = await Promise.all(
        (data || []).map(async (comment) => {
          ...
          return {
            ...
            user_instagram: comment.user_data?.instagram,
            user_linkedin: comment.user_data?.linkedin,
            replies,
  ...
  async getCommentReplies(commentId: string): Promise<Comment[]> {
    ...
    try {
      ...
      return (data || []).map(reply => ({
        ...reply,
        user_name: reply.user_data?.['Nome do cliente'] || 'Usuário',
        user_avatar: reply.user_data?.avatar_usuario || '/avatar1.png',
        user_instagram: reply.user_data?.instagram,
        user_linkedin: reply.user_data?.linkedin,
        replies: [],
        reply_count: 0
      })) as Comment[];
    } catch (error) {
